...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        def dp(s1_pointer, s2_pointer) -> bool:
            ...
            if (s1_pointer < len(s1) and s1[s1_pointer] == s3[s3_pointer]) and (s2_pointer < len(s2) and s2[s2_pointer] == s3[s3_pointer]):
...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        for j in range(n-1, -1, -1):
            dp[j] = dp[j+1] and s2[j] == s3[j+m]
            
        
        for i in range(m-1, -1, -1):
            
            dp[n] = dp[n] and (s1[i] == s3[i+n])
            
            for j in range(n-1, -1, -1):
                k = i + j
                dp[j] = (dp[j] and s1[i] == s3[k]) or (
                        dp[j+1] and s2[j] == s3[k]
                    )
        return dp[0]
